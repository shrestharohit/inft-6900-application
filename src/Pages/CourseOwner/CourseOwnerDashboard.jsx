...
const CourseOwnerDashboardContent = () => {
    ...
    // Metrics
    const totalStudents = ownerCourses.reduce((acc, c) => acc + (c.numEnrolled || 0), 0);
    const avgRating =
        ownerCourses.length > 0
            ? (ownerCourses.reduce((acc, c) => acc + (c.rating || 0), 0) /
                ownerCourses.length).toFixed(1)
            : 0;

    const announcementsCount = ownerCourses.reduce((acc, c) => {
        const published = JSON.parse(localStorage.getItem(`published_${c.id}`)) || [];
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                <div className="bg-white p-5 rounded-lg shadow text-center">
                    <h2 className="text-2xl font-bold text-[#1f2a60]">{ownerCourses.length}</h2>
                ...
                <div className="bg-white p-5 rounded-lg shadow text-center">
                    <h2 className="text-2xl font-bold text-blue-600">{announcementsCount}</h2>
